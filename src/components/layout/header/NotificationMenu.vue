<template>
  <div class="relative" ref="dropdownRef">
    ...
    >
      ...
      >
        <h5 class="text-lg font-semibold text-gray-800 dark:text-white/90">Notifiche</h5>
      ...
      <ul class="flex flex-col h-auto overflow-y-auto custom-scrollbar max-h-[350px]">
        <li v-if="notifications.length === 0" class="py-6 text-center text-gray-500 dark:text-gray-400">
        ...
        <li v-for="notification in notifications" :key="notification.id" @click="handleItemClick(notification)">
          <a
            ...
            :class="{ 'bg-brand-50 dark:bg-brand-500/10': !notification.read }"
            href="#"
          >
            <span class="block">
...
const toggleDropdown = async () => {
  ...
  if (dropdownOpen.value && notifying.value) {
    // Segna tutte le notifiche non lette come lette usando un batch per efficienza
    try {
